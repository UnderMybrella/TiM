...
public class ModTInteger extends ModInteger{
	...
	public boolean canModify (ItemStack input, ItemStack[] recipe)
	{
		...
		for(ItemStack item : recipe)
			for(int i = 0; i < modCounts.length; i++)
				if(ItemStack.areItemStacksEqual(item, (ItemStack) stacks.get(i)))
					modCounts[i]++;
		int mod = modCounts[0];
		for(int i : modCounts)
			if(i != mod)
		...
		//System.out.println((((value / max) + (value % max != 0 ? 1 : 0)) * max) + ", " + (adding + (value % max)));
		...
		if(!tags.hasKey(key))
		{
			...
			if(adding > max && (adding % max != 0))
				return false;
			int difference = (adding + (value % max)) - (adding / max);
			if(max <= 1)
				difference = adding;
	...
	public void modifyDecrease(ItemStack[] input, ItemStack tool)
	{
		...
		if (tags.hasKey(key))
		{
			...
			if(increase % max == 0)
				if(max > 1)
					decreaseModifiers(tool, ((adding + (value % max)) - (adding / max)) + 1);
				else
